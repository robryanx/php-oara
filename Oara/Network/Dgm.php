 ...
 *
 */
class Oara_Network_Dgm extends Oara_Network {
	/**
	 * Soap client.
	 */
							...
							if (count($transactionExportArray) == 23) {
								...
								if ($transactionExportArray[15] == 'Approved') {
									...
									if ($transactionExportArray[15] == 'Pending') {
										...
										if ($transactionExportArray[15] == 'Deleted') {
			...
			for ($it = 0; $it < $iteration; $it++) {
				...
				for ($i = 0; $i < count($overviewRegisters); $i++) {
					...
					for ($j = 7; $j < count($exportData) - 2; $j++) {
						...
						$transactionDateArray = Oara_Utilities::getDayFromArray($overview['merchantId'], $transactionArray, $transactionDate, true);
