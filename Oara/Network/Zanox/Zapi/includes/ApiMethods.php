...
abstract class ApiMethods 
{
     ...
     * @return     void
     */ 
    final public function seralize( $rootName, $itemArray, $attr )
    {
    	$attr['xmlns'] = 'http://api.zanox.com/namespace/' . 
    	   $this->version . '/';
    ...
    public function doSoapRequest( $service, $method, $params = array() )
    {
    	...
    	$soap = new Oara_Import_Soap_Client($this->getWsdlUrl($service), $options);
    
    	if ( empty($params['connectId']) )
    	{
        ...
        try 
        {
            $result = $soap->__call($method, array($params));
    ...
    public function doRestfulRequest ( $resource, $parameter = false, $body = false )
    {
        ...
        {
            throw new ApiClientException("No connectId given!");
        }
        ...
        if ( is_array($parameter) )
        {
            ...
            $query = http_build_query($parameter, null, ARG_SEPARATOR);
