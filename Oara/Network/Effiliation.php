...
class Oara_Network_Effiliation extends Oara_Network{
    ...
    public function getTransactionList($merchantList = null, Zend_Date $dStartDate = null, Zend_Date $dEndDate = null, $merchantMap = null){
    	...
    	$content = file_get_contents('http://api.effiliation.com/api/transaction.csv?key='.$this->_credentials["apiPassword"].'&start='.$dStartDate->toString("dd/MM/yyyy")
    								.'&end='.$dEndDate->toString("dd/MM/yyyy").'&type=datetran');
        ...
        for ($i = 1; $i < $num; $i++) {
            ...
            if (in_array((int)$transactionExportArray[2],$merchantList)){
	            ...
	            } else if ($transactionExportArray[8] == 'Attente'){
	            	$transaction['status'] = Oara_Utilities::STATUS_PENDING;
	            } else if ($transactionExportArray[8] == 'Refusé'){
