	...
	public function getTransactionList($merchantList = null, Zend_Date $dStartDate = null, Zend_Date $dEndDate = null, $merchantMap = null) {
		...
		foreach ( $invoiceList as $invoice ) {
			...
			if ($invoiceDate->compare ( $dStartDate ) >= 0 && $invoiceDate->compare ( $dEndDate ) <= 0) {
				...
				if ($invoice->project == 7 && (preg_match ( "/Taxi/", $invoice->description ) || preg_match ( "/various/", $invoice->description ))) {
					$itemCounter = 0;
					foreach ( $invoice->items as $item ) {
						
						preg_match ( "/- (.*):/", $item->description, $matches );
						if (isset ( $matches [1] )) {
							$nameAirport = trim ( $matches [1] );
						...
						if (in_array ( $merchantId, $merchantList )) {
							...
							$transaction ['unique_id'] = $invoice->invoice_id . "-" . $itemCounter;
							
							if ($invoice->reference != null) {
								$transaction ['custom_id'] = $invoice->reference;
							}
							
							$statusMap [$invoice->status] = true;
							...
							$transaction ['amount'] = Oara_Utilities::parseDouble ( $item->unitPrice );
							$transaction ['commission'] = Oara_Utilities::parseDouble ( $item->unitPrice );
							$totalTransactions [] = $transaction;
						} else {
							 echo "Merchant not found {$item->description}\n\n";
						}
