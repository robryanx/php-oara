 ...
 * @category   Oara_Network_Publisher_Ct
 * @copyright  Fubra Limited
 * @version    Release: 01.00
 *
 */
class Oara_Network_Publisher_CarTrawler extends Oara_Network {
	/**
	 * Export client.
	 ...
	 * @param $cartrawler
	 * @return Oara_Network_Publisher_Ct_Export
	 */
		...
		foreach ($exportTransactionList as $exportTransaction) {
			...
			if ($status == 'CONFIRMED') {
				...
				if ($status == 'CANCELLED') {
					...
					if ($status == 'UNCONFIRMED' || $status == 'REBOOKED' || $status == 'PENDING INVOICE') {
