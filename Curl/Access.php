...
class Oara_Curl_Access{
	 ...
	 * @return none
	 */
	public function __construct($url, array $valuesLogin, $affiliateNetwork, $groupId, $mode) {
		//Setting cookies
		$isTDorSMG = $affiliateNetwork->affjet_affiliate_network_id == 2 || $affiliateNetwork->affjet_affiliate_network_id == 18;
		$isAW = $affiliateNetwork->affjet_affiliate_network_id == 1;
		$dir = Core::getBaseDir().'/data/curl/'.$groupId.'/'.$mode.'/';
		...
		if (!$isAW){
			if ($handle = opendir($dir)) {
				...
				while (false !== ($file = readdir($handle))) {
					if ($affiliateNetwork->id == strstr($file, '_', true)){
						unlink($dir.$file);
					}
				}
				closedir($handle);
		...
		$cookieName = $affiliateNetwork->id.'_'.$affiliateNetwork->name;
		...
		while ($isTDorSMG && ($info['http_code'] == 301 || $info['http_code'] == 302)) { // redirect manually, cookies must be set, which curl does not itself
