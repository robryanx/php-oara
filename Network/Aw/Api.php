 ...
 * 
 */
class Oara_Network_Aw_Api extends Oara_Network_Base{
    /**
     * Soap client.
     */
     ...
     */
	public function __construct($affiliateWindow, $groupId, $mode)
	{
		ini_set('default_socket_timeout','120');
        $configuration = $affiliateWindow->AffiliateNetworkConfig->toArray();
        ...
        $password = Oara_Utilities::arrayFetchValue($configuration,'key','apiPassword');
        $password = Oara_Utilities::decodePassword($password['value']);
        $passwordExport = Oara_Utilities::arrayFetchValue($configuration,'key','password');
        $passwordExport = Oara_Utilities::decodePassword($passwordExport['value']);
		...
		if ($validator->isValid($user)) {
            ...
            $this->_exportClient = new Oara_Curl_Access($loginUrl, $valuesLogin, $affiliateWindow, $groupId, $mode);
