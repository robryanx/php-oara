 ...
 * 
 */
class Oara_Network_Wow_Api extends Oara_Network_Base{
    /**
     * Soap client.
     */
     ...
     */
	public function __construct($wow, $groupId, $mode)
	{
        $configuration = $wow->AffiliateNetworkConfig->toArray();
        //Reading the different parameters.
        $client = Oara_Utilities::arrayFetchValue($configuration,'key','client');
        $client = Oara_Utilities::decodePassword($client['value']);
        
        $addCode = Oara_Utilities::arrayFetchValue($configuration,'key','addcode');
        $addCode = Oara_Utilities::decodePassword($addCode['value']);
		...
		$this->_exportClient = new Oara_Curl_Access($loginUrl, $valuesLogin, $wow, $groupId, $mode);
