 ...
 *
 */
class Oara_Network_St_Export extends Oara_Network_Base{
    /**
	 ...
	 */
	public function __construct($silvertap, $groupId, $mode)
	{
		$configuration = $silvertap->AffiliateNetworkConfig->toArray();
		...
		$this->_client = new Oara_Curl_Access($loginUrl, $valuesLogin, $silvertap, $groupId, $mode);
	...
	public function getPaymentHistory($currentPage = 1){
		...
		if (PHP_OS == "WIN32" || PHP_OS == "WINNT") {
		   $pdfToTextPath = Core::getBaseDir().'/library/PdfConverter/pdftotext.exe ';
		} else {
		   // some other platform
