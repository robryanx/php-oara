 ...
 * 
 */
class Oara_Network_An_Api extends Oara_Network_Base{
    /**
     * Soap client.
     */
     ...
     */
	public function __construct($affilinet, $groupId, $mode)
	{
        $configuration = $affilinet->AffiliateNetworkConfig->toArray();
        //Reading the different parameters.
        $user = Oara_Utilities::arrayFetchValue($configuration,'key','user');
        $this->_user = Oara_Utilities::decodePassword($user['value']);
        $password = Oara_Utilities::arrayFetchValue($configuration,'key','apiPassword');
        $this->_password = Oara_Utilities::decodePassword($password['value']);
