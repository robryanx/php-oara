 ...
 * 
 */
class Oara_Network_Zn_Api extends Oara_Network_Base{
    /**
     * Soap client.
     */
     ...
     */
	public function __construct($zanox, $groupId, $mode)
	{
		
		$api = Oara_Network_Zn_Zapi_ApiClient::factory(PROTOCOL_SOAP, VERSION_2009_07_01);
 		
		$configuration = $zanox->AffiliateNetworkConfig->toArray();

        $connectId = Oara_Utilities::arrayFetchValue($configuration, 'key', 'connectId');
        $connectId = Oara_Utilities::decodePassword($connectId['value']);
        
        $secretKey = Oara_Utilities::arrayFetchValue($configuration, 'key', 'secretKey');
        $secretKey = Oara_Utilities::decodePassword($secretKey['value']);
        
        $publicKey = Oara_Utilities::arrayFetchValue($configuration, 'key', 'publicKey');
        $publicKey = Oara_Utilities::decodePassword($publicKey['value']);
